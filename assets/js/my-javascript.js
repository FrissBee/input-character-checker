...
(() => {
  ...
  DOM.inputCharacterChecker_Demo_20 = document.querySelector('input-character-checker.demo_20');
  ...
  const init = () => {
    ...
    DOM.inputCharacterChecker_Demo_14.iconField.addEventListener('event-icon-button', (e) => {
      alert('Input value: ' + e.detail + '\n\n' + 'Input length: ' + e.detail.length + '\n\nAlso look in the console');
    ...
    DOM.inputCharacterChecker_Demo_18.textField.addEventListener('event-input-textfield', (e) => {
      checkPassword(DOM.inputCharacterChecker_Demo_18, DOM.checkIcon_1, DOM.checkIcon_2, DOM.checkIcon_3);
    });
    DOM.btnCheckPassword.addEventListener('click', sendPassword);

    // 20. border color by no input
    DOM.inputCharacterChecker_Demo_20.iconField.addEventListener('event-icon-button', (e) => {
      setBorderColorError(DOM.inputCharacterChecker_Demo_20);
    });
    DOM.inputCharacterChecker_Demo_20.textField.addEventListener('event-input-textfield', (e) => {
      setBorderColorDefault(DOM.inputCharacterChecker_Demo_20);
    });
  };
  ...
  // 7. Example: ono whitespaces
  const handleTextfieldValue_5 = (e) => {
    DOM.inputCharacterChecker_Demo_5.noWhiteSpaces();
  };
  ...
  // 18. Example: check input for password
  const checkPassword = (elem, icon_1, icon_2, icon_3) => {
    // At least characters
  ...
  const sendPassword = (e) => {
    if (DOM.inputCharacterChecker_Demo_18.isPasswordCorrect({ isSpecialChar: false }) === false) {
  ...
  // 20. border color by no input
  const setBorderColorError = (elem) => {
    if (elem.textField.value.trim() === '') {
      elem.setBorderToError('1px solid red');
    }
  };

  const setBorderColorDefault = (elem) => {
    elem.setBorderToDefault();
  };
